...
export class EthereumClient {
  ...
  public async connectWalletConnect(onUri: (uri: string) => void, selectedChainId?: number) {
    ...
    const handleProviderEvents = isV1 ? this.connectWalletConnectV1.bind(this) : this.connectWalletConnectV2.bind(this)
    const [data] = await Promise.all([connect({ connector, chainId }), handleProviderEvents(connector, onUri)])
